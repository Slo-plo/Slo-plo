...
class VolunteerFragment : Fragment() {
    ...
    private fun getVolunteerList(): List<RecomendVolunteer> {
        return listOf(
            RecomendVolunteer("길거리 청소 봉사", "도심 거리 청소를 함께해요!"),
            RecomendVolunteer("노인복지센터 지원", "말벗이 되어 드리고 식사 보조도 해요."),
            RecomendVolunteer("지역 아동센터 미술 수업", "아이들과 창의력 넘치는 시간을 보내요."),
            RecomendVolunteer("동물 보호소 봉사", "유기동물들과 산책하며 돌봐줘요."),
            RecomendVolunteer("환경 보호 캠페인", "자연을 지키는 활동에 참여해요."),
            RecomendVolunteer("이웃 지원 봉사", "독거노인 분들에게 필요한 물품을 전달해요."),
            RecomendVolunteer("청소년 멘토링", "청소년들에게 진로 상담을 해줘요."),
            RecomendVolunteer("해변 청소 봉사", "해변에서 쓰레기를 청소해요.")
        )
    }
